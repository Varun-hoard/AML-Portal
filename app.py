...
class Transaction(db.Model):
    ...
    alert = db.relationship(
        "Alert", backref="transaction", uselist=False, cascade="all, delete-orphan"
    )
...
class Alert(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "triggered_rules": (
                json.loads(
                    self.triggered_rules) if self.triggered_rules else []
            ),
...
class FraudDetectionEngine:
    ...
    def detect_suspicious_transaction(sender, receiver, amount, transaction_date):
        ...
        time_threshold = transaction_date - timedelta(
            minutes=FraudDetectionEngine.TIME_WINDOW_MINUTES
        )
        ...
        if recent_transactions >= FraudDetectionEngine.MULTIPLE_TRANSACTION_THRESHOLD:
            reasons.append(
                f"Multiple transactions ({recent_transactions}) in short time"
            )
            risk_score += 35
...
def create_transaction():
    ...
    try:
        ...
        transaction_id = (
            f"TXN-{int(datetime.utcnow().timestamp())}-{data['sender_account'][-4:]}"
        )

        # Run fraud detection
        is_suspicious, risk_level, reasons = (
            FraudDetectionEngine.detect_suspicious_transaction(
                data["sender_account"],
        ...
        if is_suspicious:
            alert = Alert(
                ...
                triggered_rules=json.dumps(
                    [r.split("(")[0].strip() for r in reasons]),
            )
        ...
        logger.info(
            f"Transaction created: {transaction_id}, Suspicious: {is_suspicious}"
        )
...
def get_transactions():
    ...
    try:
        ...
        suspicious_only = request.args.get(
            "suspicious", "false").lower() == "true"
...
def get_dashboard_stats():
    ...
    try:
        ...
        suspicious_transactions = Transaction.query.filter_by(
            is_suspicious=True
        ).count()
        ...
        risk_distribution = (
            db.session.query(
                Transaction.risk_level, db.func.count(
                    Transaction.id).label("count")
            )
            ...
            jsonify(
                {
                    ...
                    "suspicious_percentage": (
                        round((suspicious_transactions /
                              total_transactions * 100), 2)
...
def get_or_update_alert(alert_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Alert updated successfully", "alert": alert.to_dict()}
            ),
            200,
        )
        ...
        for alert in alerts:
            writer.writerow(
                [
                    ...
                    alert.status,
                    (
                        ",".join(json.loads(alert.triggered_rules))
...
def get_advanced_stats():
    ...
    try:
        ...
        alert_status = (
            db.session.query(Alert.status, db.func.count(
                Alert.id).label("count"))
        ...
        alert_risk = (
            db.session.query(Alert.risk_level, db.func.count(
                Alert.id).label("count"))
        ...
        recent_suspicious = Transaction.query.filter(
            Transaction.created_at >= seven_days_ago, Transaction.is_suspicious.is_(True)
        ).count()
        ...
        detection_reasons = (
            ...
            .filter(Transaction.detection_reason.isnot(None))
            .group_by(Transaction.detection_reason)
        ...
        reasons = [
            {"reason": reason, "count": count} for reason, count in detection_reasons
        ]
            ...
            jsonify(
                {
                    ...
                    "recent_7_days": {
                        ...
                        "suspicious_percentage": (
                            round((recent_suspicious / recent_total * 100), 2)
                            if recent_total > 0
                            else 0
                        ),
                    },
...
def bulk_update_alerts():
    ...
    try:
        ...
        if not data or "alert_ids" not in data or not data.get("update_data"):
            return (
                jsonify(
                    {"error": "Missing required fields (alert_ids, update_data)"}),
                400,
            )
